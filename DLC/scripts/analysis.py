...
def distance_vel(df, debug):
    ...
    #[TEESHA] updated w to align with naming
    #likelihood_columns = [col for col in df.columns if "likelihood" in col]
    likelihood_columns = [col for col in df.columns if col.endswith("_L")]
    ...
    #[TEESHA] Replacing line above to block below for safer processing: 
    avg_dist = []
    for i in range(len(dist_df)):
        try:
            w = weights_norm.iloc[i]
            d = dist_df.iloc[i]
            # skip if weights are zero or nan
            if w.sum() == 0 or w.isna().all():
                avg_dist.append(np.nan)
            else:
                avg_dist.append(np.average(d, weights=w))
        except:
            avg_dist.append(np.nan)
    df["avg_dist_vel"] = avg_dist
...
def tail_head_angle(df, debug):
    ...
    # Try to identify head and tail
    head_x = next((col for col in x_columns if "Head" in col), None)
    tail_x = next((col for col in x_columns if "Tail" in col), None)
    head_y = next((col for col in y_columns if "Head" in col), None)
    tail_y = next((col for col in y_columns if "Tail" in col), None)

    # If not found, skip
    if not all([head_x, head_y, tail_x, tail_y]):
        print("❗️Head or Tail coordinates not found. Skipping angle calculation.")
        print("Available columns:", df.columns.tolist()) 
        df["angle"] = np.nan
        return df

    # Calculate angle
    df["angle"] = np.arctan2(df[head_y] - df[tail_y], df[head_x] - df[tail_x]) * 180 / np.pi
    df["angle"] = df["angle"].apply(lambda x: x if x >= 0 else 360 + x)
    print(df.head(5)) if debug else None
    return df
